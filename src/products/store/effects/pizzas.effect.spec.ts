...
describe('PizzaEffects', () => {
    ...
    describe('deletePizza$', () => {
        it('should work', () => {
            const action = new fromActions.DeletePizza(pizzas[0])
            const completion = new fromActions.DeletePizzaSuccess(pizzas[0])

            actions$.stream = hot('-a', { a: action })
            const expected = cold('-c', { c: completion })

            expect(effects.deletePizza$).toBeObservable(expected)
        })
    })
})
